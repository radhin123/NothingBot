let handler = async (m, { conn }) => {
   ...
   if ( user.pickaxe > 0 ) {
    ...
    m.reply(`Kamu Menambang Di ${pickRandom(['⛰️Lembah', '⛰️Goa mletre', '🏞️Sungai Selandia', '⛰️Goa texas', '...'])}\n*⚒️Hasil Tambang:* 🪙Emas: *${minim}*\n⛓️Besi: *${minam}*\n🕸️String: *${minkm}*`)
      } else conn.sendButton( m.chat, `Tunggu 🕓${timers} lagi, untuk menambang`, `${botol}`, `⬅️ Back`, `.menu`, m)
    } else conn.sendButton( m.chat, `Kamu Tidak Mempunyai *⛏️Pickaxe* untuk menambang\nBuat Lah Pickaxe menggunakan string kayu dan batu!`, footer, `Craft Pickaxe`, `.craft pickaxe`,m )
  }
